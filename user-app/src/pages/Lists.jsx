...
function MiniPosterGrid({ items }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-1 w-24 h-36 flex-shrink-0 rounded-lg overflow-hidden bg-gray-800">
      {posters.map((item, i) => (
        <img
          key={item.id}
      ...
      {posters.length === 0 && (
        <div className="col-span-2 row-span-2 flex items-center justify-center text-gray-600 text-2xl">
          📋
        </div>
      )}
    </div>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-white mb-1">My Lists</h1>
          <p className="text-gray-400">{lists.length} custom {lists.length === 1 ? 'list' : 'lists'}</p>
        </div>
        <button
      ...
      {showCreate && (
        <div className="card-glass p-5 mb-6">
          <h3 className="font-semibold text-white mb-4">Create a new list</h3>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {lists.map(list => (
            <div key={list.id} className="card-glass p-4 flex gap-4 items-center group hover:border-gray-700 transition-all">
              <MiniPosterGrid items={list.items} />
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold text-white text-lg">{list.name}</h3>
                {list.description && (
                  <p className="text-gray-400 text-sm mt-0.5 line-clamp-1">{list.description}</p>
                )}
              ...
              <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button
                  ...
                  className="text-xs text-red-500/60 hover:text-red-400 transition-colors px-2"
                >
                  Delete
                </button>
              </div>
